...
This repository is a monorepo for **Solace** — a calendar and mail application — containing a web frontend, a native mobile app, a backend API, a notifications service, the Stalwart mail stack, Gatus status page, Errex error tracking, and shared packages, structured using Bun workspaces (`apps/web`, `apps/native`, `apps/backend`, `apps/notifications`, `packages/*`). `apps/stalwart`, `apps/gatus`, and `apps/errex` are Docker/config apps, not Bun packages.
...
- **apps/errex**: Self-hosted [errex](https://github.com/TheHoltz/errex) error tracker (Sentry-SDK compatible). Railway deploys `ghcr.io/theholtz/errex:latest` (IaC `image()`); volume at `/data` (SQLite in `/data/store`); public UI at `errors.solace.onl`. Web uses `@sentry/nextjs` (`NEXT_PUBLIC_SENTRY_DSN`); API uses `@sentry/bun` (`SENTRY_DSN`); native posts Sentry-compatible envelopes to Errex over HTTP (`EXPO_PUBLIC_SENTRY_DSN`, no native Sentry SDK). No PII (`sendDefaultPii: false`). Operator docs in `apps/errex/README.md`.
...
- **Railway**: mail (Stalwart), notifications, Gatus, Errex, and Postgres are defined in `.railway/railway.ts`. Web + API are on Vercel — do not re-add them to Railway IaC. Preview with `railway config plan`; apply only when asked.
