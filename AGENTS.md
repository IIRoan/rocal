...
- **apps/errex**: Self-hosted [errex](https://github.com/TheHoltz/errex) error tracker (Sentry-SDK compatible). Railway deploys `ghcr.io/theholtz/errex:latest` (IaC `image()`); volume at `/data` (SQLite in `/data/store`); public UI at `errors.solace.onl`. Web uses `@sentry/nextjs` (`NEXT_PUBLIC_SENTRY_DSN`); API uses `@sentry/bun` (`SENTRY_DSN`). No PII (`sendDefaultPii: false`). Operator docs in `apps/errex/README.md`.
