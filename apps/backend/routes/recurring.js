...
export const recurringRoutes = new Elysia({ prefix: "/recurring" })
    ...
    try {
        ...
        const instances = RecurrenceEngine.generateInstances(mockEvent, startDate, previewEndDate, []);
        return {
    ...
    .put("/event/:id", async ({ params, body, user }) => {
    const { id } = params;
    ...
    body: t.Object({
        ...
        occurrenceDate: t.Optional(t.String({
            description: "ISO date of the specific occurrence (required for this_only and this_and_future)",
        })),
        updates: t.Object({
    ...
    .delete("/event/:id", async ({ params, query, user }) => {
    const { id } = params;
