...
export const eventsRoutes = new Elysia({ prefix: "/events" })
    ...
    if (isNaN(startDate.getTime())) {
        throw new ValidationError("Invalid start date format. Use ISO 8601 format (YYYY-MM-DDTHH:mm:ss.sssZ)");
    }
    if (isNaN(endDate.getTime())) {
        throw new ValidationError("Invalid end date format. Use ISO 8601 format (YYYY-MM-DDTHH:mm:ss.sssZ)");
    }
    ...
    for (const recurringEvent of recurringEvents) {
        try {
            ...
            if (recurringEvent.start >= startDate &&
    ...
    detail: {
        ...
        description: "Fetches authenticated user's events within the specified date range, including associated categories for efficient frontend rendering",
        security: [{ bearerAuth: [] }],
        responses: {
            200: {
    ...
    try {
        ...
        const endDate = new Date(end);
        if (isNaN(startDate.getTime())) {
            throw new ValidationError("Invalid start date format. Use ISO 8601 format", "start");
        }
        if (isNaN(endDate.getTime())) {
            throw new ValidationError("Invalid end date format. Use ISO 8601 format", "end");
        }
        if (startDate >= endDate) {
        ...
        if (body.color) {
            ...
            const isHexColor = /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/.test(body.color);
            if (!allowedColors.includes(body.color) && !isHexColor) {
                throw new ValidationError(`Color must be one of: ${allowedColors.join(", ")} or a valid hex color (e.g., #FF0000)`, "color");
            }
        }
        ...
        if (body.recurrence) {
            try {
                ...
                if (!rule) {
                    throw new ValidationError("Invalid recurrence rule format", "recurrence");
                }
                const errors = RecurrenceEngine.validateRecurrenceRule(rule);
                if (errors.length > 0) {
                    throw new ValidationError(`Recurrence rule validation failed: ${errors.join(", ")}`, "recurrence");
                }
            }
        ...
        if (body.categoryId) {
            ...
            if (!category) {
                throw new ValidationError("Invalid category or category does not belong to user", "categoryId");
            }
        }
        ...
        if (body.location && body.location.length > 255) {
            throw new ValidationError("Location cannot exceed 255 characters", "location");
        }
        ...
        if (!calendar) {
            throw new ValidationError("Invalid calendar or calendar does not belong to user", "calendarId");
        }
        ...
        if (body.reminder !== undefined && body.reminder !== null) {
            ...
            if (isNaN(reminderValue) ||
                reminderValue < 0 ||
                reminderValue > 43200) {
                throw new ValidationError("Reminder must be a number between 0 and 43200 minutes", "reminder");
            }
        ...
        try {
            if (body.reminder && body.reminder > 0) {
                ...
                if (userSettings?.emailNotifications !== false) {
                    ...
                    const notificationTime = NotificationCalculator.calculateNotificationTime(startDate, body.reminder);
    ...
    detail: {
        ...
        description: "Creates a new calendar event for the authenticated user with proper validation",
        security: [{ bearerAuth: [] }],
        responses: {
            200: {
    ...
    try {
        ...
        try {
            ...
            if (timeChanged || reminderChanged) {
                ...
                if (notificationConfigs.length > 0) {
                    ...
                    for (const config of notificationConfigs) {
                        const notificationTime = NotificationCalculator.calculateNotificationTime(finalStartDate, config.minutesBefore);
                        await prisma.eventNotification.create({
                            data: {
                                eventId: id,
            ...
            console.error("Failed to update notifications with enhanced service:", notificationError);
        ...
        if (error.code === "P2025" ||
            ...
            throw new ValidationError("Event was modified by another process. Please refresh and try again.");
        }
        throw error;
    }
}, {
    ...
    detail: {
        ...
        description: "Updates an existing calendar event with ownership verification and optimistic locking to prevent concurrent update conflicts",
        security: [{ bearerAuth: [] }],
        responses: {
            200: {
    ...
    try {
        ...
        console.log(`✓ Deleted ${deletedNotifications.count} notifications for event ${id}`);
    ...
    detail: {
        ...
        description: "Deletes a calendar event with proper user ownership verification",
        security: [{ bearerAuth: [] }],
        responses: {
            200: {
    ...
    try {
        ...
        if (events.length !== eventIds.length) {
            throw new ValidationError("Some events not found or access denied", "eventIds");
        }
        ...
        if (syncedEvents.length > 0) {
            throw new ValidationError(`Cannot modify synced events. The following synced events are read-only: ${syncedEvents.map(e => e.title).join(', ')}`);
        }
        let result;
        switch (action) {
            case "move":
                if (!targetCalendarId) {
                    throw new ValidationError("Target calendar ID is required for move operation", "targetCalendarId");
                }
                ...
                if (!targetCalendar) {
                    throw new ValidationError("Target calendar not found or access denied", "targetCalendarId");
                }
            ...
            case "delete":
                ...
                console.log(`✓ Deleted ${deletedNotifications.count} notifications for ${eventIds.length} events`);
                // Clean up notification logs
                await prisma.notificationLog.deleteMany({
                    where: { eventId: { in: eventIds } },
                });
                ...
                for (const event of events) {
                    ...
                    try {
                        if (event.reminder && event.reminder > 0) {
                            ...
                            if (userSettings?.emailNotifications !== false) {
                                const { NotificationCalculator } = await import("../lib/notification-calculator");
                                const notificationTime = NotificationCalculator.calculateNotificationTime(duplicated.start, event.reminder);
                                await prisma.eventNotification.create({
                                    data: {
                                ...
                                console.log(`✓ Created notification for duplicated event ${duplicated.id}`);
                            }
                        }
                    }
                    catch (notificationError) {
                        console.error("Failed to create notifications for duplicated event:", notificationError);
            ...
            default:
                throw new ValidationError("Invalid action. Use 'move', 'delete', or 'duplicate'", "action");
        }
    }
    catch (error) {
    ...
    auth: true,
    body: t.Object({
        action: t.Union([t.Literal("move"), t.Literal("delete"), t.Literal("duplicate")], {
