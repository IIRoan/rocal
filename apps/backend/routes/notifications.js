...
function rateLimit(limit) {
    return ({ request, set, user }) => {
        ...
        if (current.count >= limit.requests) {
            set.status = 429;
            set.headers["Retry-After"] = Math.ceil((current.resetTime - now) / 1000).toString();
            throw new ValidationError(`Rate limit exceeded. Try again in ${Math.ceil((current.resetTime - now) / 1000)} seconds.`);
        }
        current.count++;
    };
}
...
function validateNotificationConfig(config) {
    if (!config.notificationType ||
        ...
        throw new ValidationError("Invalid notification type. Must be 'email' or 'browser'.");
    }
    ...
    if (config.minutesBefore > 43200) {
        // 30 days in minutes
        throw new ValidationError("minutesBefore cannot exceed 30 days (43200 minutes).");
    }
 ...
 */
async function validateEventOwnership(eventId, userId) {
...
export const notificationsRoutes = new Elysia({ prefix: "/notifications" })
    .get("/event/:eventId", async ({ params, user, request, set }) => {
    try {
        // Apply rate limiting
    ...
    detail: {
        ...
        description: "Retrieves all notification settings for a specific event with enhanced validation and rate limiting",
        security: [{ bearerAuth: [] }],
        responses: {
            200: {
    ...
    .put("/event/:eventId", async ({ params, body, user, request, set }) => {
    try {
        // Apply rate limiting
        ...
        const configKeys = notifications.map((n) => `${n.notificationType}-${n.minutesBefore}`);
        const uniqueKeys = new Set(configKeys);
        if (configKeys.length !== uniqueKeys.size) {
            throw new ValidationError("Duplicate notification configurations are not allowed");
        }
        ...
        for (const config of notifications) {
            if (config.isEnabled) {
                const notificationTime = new Date(event.start.getTime() - config.minutesBefore * 60 * 1000);
    ...
    detail: {
        ...
        description: "Updates all notification settings for a specific event using the enhanced notification service with comprehensive validation",
        security: [{ bearerAuth: [] }],
        responses: {
    ...
    .delete("/event/:eventId", async ({ params, user, request, set }) => {
    try {
        // Apply rate limiting
        rateLimit(RATE_LIMITS.UPDATE_NOTIFICATIONS)({ request, set, user });
        const { eventId } = params;
    ...
    detail: {
        ...
        description: "Deletes all notification settings for a specific event using the enhanced notification service",
        security: [{ bearerAuth: [] }],
        responses: {
            200: {
