...
export const settingsRoutes = new Elysia({ prefix: "/settings" })
    ...
    if (body.workingHoursStart !== undefined &&
        ...
        if (body.workingHoursStart >= body.workingHoursEnd) {
            throw new ValidationError("Working hours start must be before working hours end", "workingHoursStart");
        }
    }
    ...
    if (body.workingDays) {
        try {
            ...
            if (!Array.isArray(workingDays) ||
                !workingDays.every((day) => typeof day === "number" && day >= 0 && day <= 6)) {
                throw new ValidationError("Working days must be a JSON array of numbers 0-6", "workingDays");
            }
        }
        catch (error) {
            throw new ValidationError("Invalid working days format - must be valid JSON array", "workingDays");
        }
    }
    ...
    if (body.defaultCalendarId) {
        ...
        if (!calendar) {
            throw new ValidationError("Invalid default calendar or calendar does not belong to user", "defaultCalendarId");
        }
    }
