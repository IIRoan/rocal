...
export const calendarsRoutes = new Elysia({ prefix: "/calendars" })
    ...
    if (!name?.trim()) {
        throw new ValidationError("Calendar name is required and cannot be empty", "name");
    }
    ...
    if (existingCalendar) {
        throw new ValidationError("A calendar with this name already exists", "name");
    }
    ...
    body: t.Object({
        ...
        color: t.String({
            description: "Calendar color (blue, orange, violet, rose, emerald, or hex color like #FF0000)",
        }),
    ...
    if (calendarCount <= 1) {
        throw new ValidationError("Cannot delete the last calendar. Create another calendar first.", "calendarId");
    }
    ...
    if (events.length > 0) {
        if (action === "move_events") {
            if (!targetCalendarId) {
                throw new ValidationError("Target calendar ID is required when moving events", "targetCalendarId");
            }
            ...
            if (targetCalendarId === id) {
                throw new ValidationError("Cannot move events to the same calendar being deleted", "targetCalendarId");
            }
    ...
    query: t.Object({
        action: t.Optional(t.Union([
            ...
            description: "What to do with events: delete_events (default), or move_events",
        })),
