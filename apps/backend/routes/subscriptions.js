...
export const subscriptionsRoute = new Elysia()
    ...
    .post('/subscriptions', async ({ body, user }) => {
    ...
    .put('/subscriptions/:id', async ({ params, body, user }) => {
    const { id } = params;
    ...
    .delete('/subscriptions/:id', async ({ params, user, query }) => {
    const { id } = params;
    ...
    .post('/subscriptions/:id/sync', async ({ params, user }) => {
    const { id } = params;
    const subscription = await prisma.calendarSubscription.findFirst({
        where: {
            id,
    ...
    .post('/subscriptions/import-ics', async ({ body, user }) => {
    ...
    for (const parsedEvent of parseResult.events) {
        try {
            ...
            const eventData = convertParsedEventToCalendarEvent(parsedEvent, user.id, calendarId
...
export async function syncCalendarSubscription(subscription) {
    ...
    try {
        ...
        const currentEventsByUid = new Map(currentEvents.map(event => [event.externalId, event]));
        ...
        for (const parsedEvent of parseResult.events) {
            ...
            if (!existingEvent) {
                // Create new event
                const eventData = convertParsedEventToCalendarEvent(parsedEvent, subscription.userId, subscription.calendarId, subscription.id);
