...
function convertTimezoneToUserTimezone(date, sourceTimezone, userTimezone) {
...
function mapICSTimezoneToIANA(icsTimezone) {
...
export function parseICSFile(icsContent, userTimezone = 'UTC') {
    const result = {
        events: [],
        errors: []
    };
...
function parseVEvent(vEvent, userTimezone = 'UTC', calendarTimezone) {
    try {
...
function parseRRule(rrule) {
    ...
    // Also handle legacy byday format for string-based parsing
        ...
        recurrenceRule.byWeekDay = weekdays.map((day) => {
...
export function convertParsedEventToCalendarEvent(parsedEvent, userId, calendarId, subscriptionId) {
    return {
...
export function isEventModified(existing, parsed) {
