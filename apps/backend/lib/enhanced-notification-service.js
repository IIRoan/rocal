...
import { NotificationCalculator, } from "./notification-calculator";
import { Resend } from "resend";
import { render } from "@react-email/render";
// Conditional import to avoid Next.js build issues
let EventReminderEmail;
...
export class EnhancedNotificationService {
    ...
    start(options) {
        ...
        try {
            ...
            console.log(`⏰ Aligning notification timer to minute boundary. Current time: ${now.toISOString()}, delay: ${msUntilNextMinute}ms`);
            ...
            if (options?.enableAutomaticCleanup !== false) {
                ...
                this.scheduleAutomaticCleanup(options?.cleanupIntervalHours || 24, options?.cleanupRetentionDays || 30);
            }
            console.log("✅ Enhanced notification service started successfully");
            console.log(`📊 Service status: Running=${this.isRunning}, RetryQueue=${this.retryQueue.size}, AutoCleanup=${!!this.cleanupTimer}`);
    ...
    async performInitialHealthCheck() {
        try {
            ...
            if (pendingCount > 0) {
                console.log(`⚠️ Found ${pendingCount} overdue notifications that will be processed`);
            }
            ...
            await this.executeWithDatabaseRetry(async () => {
                return await prisma.notificationLog.create({
                    data: {
                        ...
                        notificationType: "system",
                        minutesBefore: 0,
                        status: "service_started: Enhanced notification service started successfully",
                        sentAt: new Date(),
                    },
                });
            });
    ...
    stop() {
        ...
        try {
            ...
            console.log(`📊 Final statistics: Processed=${this.processedCount}, Failed=${this.failedCount}, RetryQueue=${this.retryQueue.size}`);
     ...
     */
    async createNotificationsForEvent(eventId, eventStart, notifications) {
        try {
            ...
            if (actualEventStart < now) {
                console.log(`⏭️ Event ${eventId} is in the past, skipping notification creation`);
                return {
                    created: [],
            ...
            for (const config of notifications) {
                try {
                    ...
                    const result = NotificationCalculator.calculateNotificationTimeWithValidation(actualEventStart, config.minutesBefore);
                    if (!result.isValid) {
                        skipped.push({
                            config,
                    ...
                    console.log(`✅ Created ${config.notificationType} notification for event ${eventId}: ` +
                    ...
                    console.error(`❌ Failed to create notification for event ${eventId}:`, errorMessage);
                }
            }
            console.log(`✅ Created notifications for event ${eventId}: ${created.length} created, ${skipped.length} skipped`);
     ...
     * @returns Result with created notifications and any skipped ones
     */
    async updateNotificationsForEvent(eventId, eventStart, notifications) {
        const maxRetries = 3;
        let lastError = null;
        // Implement retry logic for concurrent update handling
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                return await this.updateNotificationsForEventWithRetry(eventId, eventStart, notifications, attempt);
            }
            catch (error) {
                ...
                if (this.isRetryableUpdateError(error) && attempt < maxRetries) {
                    ...
                    console.warn(`⚠️ Update attempt ${attempt} failed for event ${eventId}, retrying in ${delay}ms: ${lastError.message}`);
                    await new Promise((resolve) => setTimeout(resolve, delay));
                    continue;
                }
                // Non-retryable error or max retries reached
                console.error(`❌ Failed to update notifications for event ${eventId} after ${attempt} attempts:`, lastError.message);
                throw lastError;
            }
        }
     ...
     * @returns Result with created notifications and any skipped ones
     */
    async updateNotificationsForEventWithRetry(eventId, eventStart, notifications, attempt) {
        try {
            ...
            const actualEventStart = eventStart || event.start;
            if (actualEventStart < now) {
                console.log(`🧹 Event ${eventId} moved to past date, cleaning up future notifications`);
            ...
            const result = await this.executeWithDatabaseRetry(async () => {
                return await prisma.$transaction(async (tx) => {
                    ...
                    console.log(`✓ Deleted ${deletedCount.count} existing notifications for event ${eventId} (attempt ${attempt})`);
                    ...
                    for (const config of notifications) {
                        try {
                            // Calculate exact notification time
                            const calcResult = NotificationCalculator.calculateNotificationTimeWithValidation(actualEventStart, config.minutesBefore);
            ...
            console.log(`✅ Updated notifications for event ${eventId} (attempt ${attempt}): ` +
                `${result.created.length} created, ${result.skipped.length} skipped`);
            return result;
        }
        catch (error) {
            console.error(`❌ Update attempt ${attempt} failed for event ${eventId}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async deleteNotificationsForEvent(eventId) {
        try {
            ...
            if (!event) {
                console.warn(`Event with ID ${eventId} not found, but proceeding with notification cleanup`);
            }
            ...
            for (const [notificationId, retryInfo] of this.retryQueue.entries()) {
                if (retryInfo.eventId === eventId) {
                    ...
                    console.log(`🗑️ Removed notification ${notificationId} from retry queue`);
                }
            }
            console.log(`✅ Deleted ${result.count} notifications for event ${eventId}`);
            return result.count;
        }
        catch (error) {
     ...
     */
    async handleEventMovedToPast(eventId, eventStart) {
        try {
            const now = new Date();
            if (eventStart >= now) {
                console.log(`⏭️ Event ${eventId} is not in the past, no cleanup needed`);
                return 0;
            }
            console.log(`🧹 Cleaning up notifications for event ${eventId} moved to past date: ${eventStart.toISOString()}`);
            ...
            console.log(`✅ Cleaned up ${result.count} future notifications and ${removedFromRetryQueue} retry queue entries for past event ${eventId}`);
            return result.count;
        }
        catch (error) {
    ...
    async processScheduledNotifications() {
        ...
        try {
            ...
            if (notificationsToSend.length > 0) {
                console.log(`🔍 Processing ${notificationsToSend.length} scheduled notifications for ${currentMinute.toISOString()}`);
            }
            ...
            for (const notification of notificationsToSend) {
                try {
                    ...
                    console.error(`❌ Error processing notification ${notification.id} for event "${notification.event.title}": ${errorMessage}`);
                }
            }
            ...
            if (notificationsToSend.length > 0 ||
                processedThisRun > 0 ||
                failedThisRun > 0) {
                console.log(`✅ Notification processing complete: ${processedThisRun} sent, ${failedThisRun} failed, ${skippedThisRun} skipped`);
            }
            ...
            this.failedCount++;
            const errorMessage = error instanceof Error ? error.message : "Unknown error";
            console.error("❌ Critical error in background notification processing:", errorMessage);
            // Enhanced system error logging with recovery information
            this.errors.push({
                notificationId: "system",
                eventId: "system",
     ...
     */
    async handleNotificationFailure(notification, result) {
        ...
        this.errors.push({
            ...
            timestamp: new Date(),
            retryCount,
            nextRetryAt: result.shouldRetry && retryCount <= maxRetries
        ...
        await this.logNotification(notification.eventId, notification.event.userId, notification.notificationType, notification.minutesBefore, retryCount <= maxRetries && result.shouldRetry
            ? "retry_scheduled"
            : "failed");
        // Schedule retry if appropriate
        if (result.shouldRetry && retryCount <= maxRetries) {
            const nextRetryAt = new Date(Date.now() + (result.retryAfterMinutes || 5) * 60 * 1000);
            ...
            console.error(`❌ Notification ${notification.id} failed permanently after ${retryCount} attempts: ${result.error}`);
            // Remove from retry queue if it was there
            this.retryQueue.delete(notification.id);
        }
    }
     ...
     */
    async processRetryQueueWithErrorHandling() {
        const now = new Date();
        ...
        console.log(`🔄 Processing ${retriesToProcess.length} notification retries`);
        ...
        for (const notificationId of retriesToProcess) {
            ...
            try {
                ...
                if (!notification) {
                    console.warn(`⚠️ Notification ${notificationId} not found for retry, removing from queue`);
                    this.retryQueue.delete(notificationId);
                    continue;
                }
                ...
                if (notification.isSent ||
                    ...
                    console.log(`⏭️ Skipping retry for notification ${notificationId} - no longer valid`);
                    this.retryQueue.delete(notificationId);
                ...
                console.error(`❌ Error processing retry for notification ${notificationId}:`, error);
                ...
                const shouldContinueRetrying = this.shouldRetryError(error) && updatedRetryInfo.retryCount <= 3;
                if (shouldContinueRetrying) {
                    ...
                    await this.logNotificationPermanentFailure(notificationId, updatedRetryInfo);
                }
            }
        }
    }
     ...
     */
    async logProcessingMetrics(processed, failed, skipped) {
        try {
            await this.executeWithDatabaseRetry(async () => {
                return await prisma.notificationLog.create({
                    data: {
     ...
     */
    async logNotificationPermanentFailure(notificationId, retryInfo) {
        try {
            await this.executeWithDatabaseRetry(async () => {
                return await prisma.notificationLog.create({
                    data: {
    ...
    async sendNotification(notification) {
        ...
        console.log(`📧 Sending ${notification.notificationType} notification for event "${event.title}" to ${user.email}`);
        try {
            ...
            if (notification.notificationType === "email" &&
                ...
                console.log(`⏭️ Skipping email notification - user has email notifications disabled`);
                await this.markNotificationAsSent(notification.id, "skipped");
                await this.logNotification(event.id, user.id, notification.notificationType, notification.minutesBefore, "skipped", "User has email notifications disabled");
                return;
            }
            if (notification.notificationType === "browser" &&
                userSettings?.browserNotifications === false) {
                console.log(`⏭️ Skipping browser notification - user has browser notifications disabled`);
                await this.markNotificationAsSent(notification.id, "skipped");
                await this.logNotification(event.id, user.id, notification.notificationType, notification.minutesBefore, "skipped", "User has browser notifications disabled");
                return;
            }
            // Send the notification based on type
            if (notification.notificationType === "email") {
                await this.sendEmailNotification(event, user, notification.minutesBefore);
            }
            else if (notification.notificationType === "browser") {
                await this.sendBrowserNotification(event, user, notification.minutesBefore);
            }
            // Mark as sent
            await this.markNotificationAsSent(notification.id, "sent");
            // Log the successful notification
            await this.logNotification(event.id, user.id, notification.notificationType, notification.minutesBefore, "sent");
            console.log(`✅ Successfully sent ${notification.notificationType} notification for event "${event.title}"`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : "Unknown error";
            console.error(`❌ Failed to send notification for event "${event.title}":`, errorMessage);
            // Log the failed notification (but don't mark as sent so it can be retried)
            await this.logNotification(event.id, user.id, notification.notificationType, notification.minutesBefore, "failed", errorMessage);
            throw error;
        }
    }
    /**
     ...
     */
    async sendEmailNotification(event, user, minutesBefore) {
        ...
        try {
            ...
            const emailContent = await this.generateEnhancedEmailContent(event, user, minutesBefore);
            ...
            console.log(`✅ Email notification sent successfully to ${user.email} for event "${event.title}" - Resend ID: ${result.data?.id}`);
            // Log successful delivery metrics
            this.logEmailDeliveryMetrics(user.id, event.id, "success", result.data?.id);
        }
        catch (error) {
            // Enhanced error handling with specific error types
            const enhancedError = this.enhanceEmailError(error, user, event);
            console.error(`❌ Failed to send email notification to ${user.email} for event "${event.title}":`, enhancedError.message);
            // Log failed delivery metrics
            this.logEmailDeliveryMetrics(user.id, event.id, "failed", undefined, enhancedError.message);
            throw enhancedError;
        }
    }
    /**
     ...
     */
    async generateEnhancedEmailContent(event, user, minutesBefore) {
        try {
            ...
            const emailHTML = await this.renderEmailTemplateWithCaching(event, user, formattedDetails, userTheme);
            ...
            throw new Error(`Email content generation failed: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    /**
     ...
     */
    formatEventDetailsForEmail(event, timeFormat, timezone, minutesBefore) {
     ...
     */
    async renderEmailTemplateWithCaching(event, user, formattedDetails, userTheme) {
        // Create cache key for template rendering (for performance)
        const cacheKey = this.generateTemplateCacheKey(event, user, formattedDetails, userTheme);
     ...
     */
    generatePlainTextEmail(event, formattedDetails) {
        const lines = [
        ...
        lines.push("", "---", "This reminder was sent because you have email notifications enabled.", "You can manage your notification preferences in your calendar settings.");
        return lines.join("\n");
    }
    /**
     ...
     */
    generateEmailSubject(event, minutesBefore) {
     ...
     */
    async sendEmailWithRetryLogic(emailData) {
        ...
        for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
            try {
                ...
                if (attempt <= maxRetries) {
                    const delay = Math.pow(2, attempt - 1) * 1000; // Exponential backoff
                    console.warn(`⚠️ Email send attempt ${attempt} failed, retrying in ${delay}ms: ${lastError.message}`);
                    await new Promise((resolve) => setTimeout(resolve, delay));
                }
            }
        }
     ...
     */
    enhanceEmailError(error, user, event) {
        ...
        if (errorLower.includes("authentication") ||
            ...
            return new Error("Email service authentication failed - check RESEND_API_KEY");
        }
        if (errorLower.includes("network") || errorLower.includes("connection")) {
            return new Error(`Network error sending email to ${user.email}: ${originalMessage}`);
        }
        if (errorLower.includes("timeout")) {
            return new Error(`Email delivery timeout for ${user.email}`);
        }
        // Generic enhanced error
        return new Error(`Email delivery failed for event "${event.title}" to ${user.email}: ${originalMessage}`);
    }
    /**
     ...
     */
    async executeWithDatabaseRetry(operation, maxRetries = 3) {
        let lastError = null;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (this.isRetryableDatabaseError(error) && attempt < maxRetries) {
                    ...
                    console.warn(`⚠️ Database operation attempt ${attempt} failed, retrying in ${delay}ms: ${lastError.message}`);
                    await new Promise((resolve) => setTimeout(resolve, delay));
                    continue;
                }
                // Non-retryable error or max retries reached
     ...
     */
    async handleConcurrentNotificationUpdate(eventId, updateOperation) {
        ...
        while (this.updateLocks.has(lockKey) &&
            ...
            if (Date.now() - startTime > maxWaitTime) {
                throw new Error(`Timeout waiting for notification update lock for event ${eventId}`);
            }
    ...
    async batchUpdateNotifications(updates) {
        ...
        for (let i = 0; i < updates.length; i += batchSize) {
            ...
            const batchPromises = batch.map(async (update) => {
                try {
                    const result = await this.handleConcurrentNotificationUpdate(update.eventId, () => this.updateNotificationsForEvent(update.eventId, update.eventStart, update.notifications));
                    return { ...result, eventId: update.eventId };
                }
                catch (error) {
                    console.error(`❌ Failed to update notifications for event ${update.eventId}:`, error);
                    return {
     ...
     */
    formatTimeUntilEventEnhanced(eventStart) {
        const now = new Date();
     ...
     */
    formatTimeWithPreference(date, timeFormat, timezone) {
     ...
     */
    formatReminderText(minutesBefore) {
        if (minutesBefore < 60) {
     ...
     */
    generateTemplateCacheKey(event, user, formattedDetails, userTheme) {
     ...
     */
    logEmailDeliveryMetrics(userId, eventId, status, resendId, errorMessage) {
        const logData = {
            userId,
            eventId,
            status,
     ...
     * @param minutesBefore - Minutes before the event
     */
    async sendBrowserNotification(event, user, minutesBefore) {
     ...
     */
    async markNotificationAsSent(notificationId, status) {
     ...
     */
    async logNotification(eventId, userId, notificationType, minutesBefore, status, errorMessage) {
        try {
            ...
            const logLevel = status === "sent" ? "info" : status === "failed" ? "error" : "warn";
     ...
     * @returns Formatted time string
     */
    formatTimeUntilEvent(eventStart) {
        const now = new Date();
        const timeDiff = eventStart.getTime() - now.getTime();
        const minutesUntil = Math.floor(timeDiff / (1000 * 60));
        const hoursUntil = Math.floor(minutesUntil / 60);
     ...
     */
    getRetryQueueInfo() {
        return Array.from(this.retryQueue.values()).sort((a, b) => a.nextRetryAt.getTime() - b.nextRetryAt.getTime());
    }
    /**
    ...
    async cleanupOldNotifications(retentionDays = 30) {
        ...
        console.log(`🧹 Starting notification cleanup - retaining logs newer than ${retentionCutoff.toISOString()}`);
        try {
            ...
            console.log(`✅ Deleted ${deletedLogsResult.count} old notification logs`);
            ...
            const deletedNotificationsResult = await prisma.eventNotification.deleteMany({
                where: {
                    isSent: true,
                    createdAt: {
                        lt: retentionCutoff,
            ...
            console.log(`✅ Deleted ${deletedNotificationsResult.count} old sent notifications`);
            ...
            console.log(`🎯 Cleanup completed in ${cleanupDuration}ms - Logs: ${deletedLogsResult.count}, Notifications: ${deletedNotificationsResult.count}`);
            ...
            const cleanupDuration = Date.now() - startTime;
            const errorMessage = error instanceof Error ? error.message : "Unknown error";
            console.error(`❌ Cleanup failed after ${cleanupDuration}ms:`, errorMessage);
            // Log cleanup failure
            await this.logCleanupStatistics({
                deletedLogs: 0,
        ...
        try {
            ...
            for (const table of notificationTables) {
                try {
                    ...
                    console.warn(`⚠️ Failed to analyze table ${table}:`, error instanceof Error ? error.message : "Unknown error");
                }
            }
            ...
            if (shouldReindex) {
                ...
                for (const index of indexesToReindex) {
                    try {
                        ...
                        console.warn(`⚠️ Failed to reindex ${index}:`, error instanceof Error ? error.message : "Unknown error");
                    }
                }
            }
            ...
            if (shouldVacuum) {
                for (const table of ["notification_log", "event_notification"]) {
                    try {
                        ...
                        console.warn(`⚠️ Failed to vacuum table ${table}:`, error instanceof Error ? error.message : "Unknown error");
                    }
                }
            }
            ...
            console.error("❌ Database maintenance failed:", error instanceof Error ? error.message : "Unknown error");
            return maintenanceResults;
        }
    }
    /**
    ...
    async shouldPerformReindexing() {
        try {
            ...
            const hasSignificantData = notificationLogCount > 10000 || eventNotificationCount > 5000;
     ...
     */
    async logCleanupStatistics(stats) {
        try {
            ...
            console.error("Failed to log cleanup statistics:", error instanceof Error ? error.message : "Unknown error");
     ...
     */
    scheduleAutomaticCleanup(intervalHours = 24, retentionDays = 30) {
        // Clear any existing cleanup timer
        if (this.cleanupTimer) {
            clearInterval(this.cleanupTimer);
        }
        const intervalMs = intervalHours * 60 * 60 * 1000;
        console.log(`⏰ Scheduling automatic cleanup every ${intervalHours} hours with ${retentionDays} day retention`);
        this.cleanupTimer = setInterval(async () => {
            try {
                console.log("🕐 Running scheduled notification cleanup...");
                const result = await this.cleanupOldNotifications(retentionDays);
                console.log(`✅ Scheduled cleanup completed: ${result.deletedLogs} logs, ${result.deletedNotifications} notifications deleted in ${result.cleanupDuration}ms`);
            }
            catch (error) {
                console.error("❌ Scheduled cleanup failed:", error instanceof Error ? error.message : "Unknown error");
            }
        }, intervalMs);
        ...
        setTimeout(async () => {
            try {
                ...
                console.error("❌ Initial cleanup failed:", error instanceof Error ? error.message : "Unknown error");
            }
    ...
    async getCleanupMetrics() {
        try {
            ...
            const [notificationLogCount, eventNotificationCount, oldestLog, oldestNotification, recentCleanupLogs,] = await Promise.all([
            ...
            const cleanupHistory = recentCleanupLogs
                ...
                try {
                    ...
                    const statusMatch = log.status.match(/deleted_logs=(\d+), deleted_notifications=(\d+), duration=(\d+)ms/);
                    if (statusMatch &&
                        statusMatch[1] &&
                        statusMatch[2] &&
                        statusMatch[3]) {
            ...
            if (notificationLogCount > 100000) {
                recommendedActions.push("Consider reducing retention period - notification log table is large");
            }
            if (eventNotificationCount > 50000) {
                recommendedActions.push("High number of event notifications - consider cleanup of old sent notifications");
            }
            const daysSinceOldestLog = oldestLog
                ? Math.floor((Date.now() - oldestLog.createdAt.getTime()) / (1000 * 60 * 60 * 24))
                : 0;
            if (daysSinceOldestLog > 90) {
                recommendedActions.push(`Oldest logs are ${daysSinceOldestLog} days old - consider running cleanup`);
            }
            if (cleanupHistory.length === 0) {
                recommendedActions.push("No recent cleanup operations found - consider enabling automatic cleanup");
            }
            ...
            throw new Error(`Failed to retrieve cleanup metrics: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    /**
    ...
    async getNotificationStatistics() {
        try {
            ...
            return {
                ...
                errorSummary: Array.from(errorSummary.entries()).map(([error, info]) => ({
                    error,
                    count: info.count,
     ...
     */
    async getNotificationsForEvent(eventId) {
        try {
     ...
     */
    validateNotificationConfigs(eventStart, notifications) {
        return notifications.map((config) => {
            try {
                const result = NotificationCalculator.calculateNotificationTimeWithValidation(eventStart, config.minutesBefore);
                return {
                    config,
     ...
     */
    async createNotificationsForRecurringEvent(eventId, notifications, startDate, endDate) {
        try {
            ...
            try {
                ...
                for (const instance of instances) {
                    if (!instance.isOriginal) {
                        ...
                        const result = await this.createNotificationsForEvent(eventId, instance.date, notifications);
                        allCreated.push(...result.created);
                        allSkipped.push(...result.skipped);
                    }
                }
                console.log(`✓ Created ${allCreated.length} notifications for ${instances.length} occurrences of recurring event ${eventId}`);
                return {
                    created: allCreated,
                    skipped: allSkipped,
                };
