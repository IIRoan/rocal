...
export class RecurrenceEngine {
     ...
     */
    static parseRecurrenceRule(recurrenceJson) {
        try {
     ...
     */
    static createRecurrenceRule(rule) {
        return JSON.stringify(rule);
    }
    /**
     * Generate recurring instances for a given date range
     */
    static generateInstances(baseEvent, rangeStart, rangeEnd, exceptions = []) {
        ...
        if (currentDate >= rangeStart && currentDate <= rangeEnd) {
            const isDeleted = exceptions.some((ex) => isSameDay(ex.exceptionDate, currentDate) && ex.type === "deleted");
            if (isDeleted) {
        ...
        switch (rule.frequency) {
            ...
            case "yearly":
                if (rule.byMonth && rule.byMonth.length > 0) {
                    ...
                    if (rule.byMonthDay && rule.byMonthDay.length > 0) {
                        nextDate.setDate(Math.min(rule.byMonthDay[0], this.getDaysInMonth(nextDate)));
                    }
                    return nextDate;
                }
                else {
    ...
    static validateRecurrenceRule(rule) {
        ...
        else if (!["daily", "weekly", "monthly", "yearly"].includes(rule.frequency)) {
        ...
        if (rule.byWeekDay) {
            ...
            else if (!Array.isArray(rule.byWeekDay) ||
                rule.byWeekDay.some((d) => d < 0 || d > 6)) {
                errors.push("byWeekDay must be an array of numbers 0-6 (Sunday-Saturday)");
            }
        }
        if (rule.byMonthDay) {
            if (rule.frequency !== "monthly" && rule.frequency !== "yearly") {
                errors.push("byMonthDay can only be used with monthly or yearly frequency");
            }
            else if (!Array.isArray(rule.byMonthDay) ||
        ...
        if (rule.byMonth) {
            ...
            else if (!Array.isArray(rule.byMonth) ||
     ...
     */
    static getRecurrenceDescription(rule) {
