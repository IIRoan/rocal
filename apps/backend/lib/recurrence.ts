...
export class RecurrenceEngine {
  ...
  static generateInstances(
    ...
    rangeStart: Date,
    rangeEnd: Date,
    exceptions: Array<{ exceptionDate: Date; type: 'modified' | 'deleted' }> = []
  ): RecurrenceInstance[] {
    ...
    if (currentDate >= rangeStart && currentDate <= rangeEnd) {
      const hasException = exceptions.some(ex => isSameDay(ex.exceptionDate, currentDate));
      if (!hasException) {
        instances.push({
  ...
  static validateRecurrenceRule(rule: RecurrenceRule): string[] {
    ...
    if (rule.byWeekDay) {
      ...
      } else if (!Array.isArray(rule.byWeekDay) || rule.byWeekDay.some(d => d < 0 || d > 6)) {
    ...
    if (rule.byMonthDay) {
      ...
      } else if (!Array.isArray(rule.byMonthDay) || rule.byMonthDay.some(d => d < 1 || d > 31)) {
    ...
    if (rule.byMonth) {
      ...
      } else if (!Array.isArray(rule.byMonth) || rule.byMonth.some(m => m < 1 || m > 12)) {
