...
export class NotificationCalculator {
     ...
     */
    static calculateNotificationTime(eventStart, minutesBefore) {
     ...
     */
    static calculateNotificationTimeWithValidation(eventStart, minutesBefore, currentTime = new Date()) {
        try {
            const notificationTime = this.calculateNotificationTime(eventStart, minutesBefore);
            const isValid = this.validateNotificationTime(notificationTime, currentTime);
            if (!isValid) {
                return {
                    notificationTime,
                    isValid: false,
     ...
     */
    static validateNotificationTime(notificationTime, currentTime = new Date()) {
     ...
     */
    static calculateMultipleNotificationTimes(eventStart, notificationConfigs, currentTime = new Date()) {
        return notificationConfigs.map((config) => ({
            ...this.calculateNotificationTimeWithValidation(eventStart, config.minutesBefore, currentTime),
            config,
        }));
    }
    /**
     ...
     */
    static isEventInFuture(eventStart, currentTime = new Date()) {
        if (!eventStart || isNaN(eventStart.getTime())) {
            return false;
        }
        if (!currentTime || isNaN(currentTime.getTime())) {
     ...
     */
    static getMaxValidMinutesBefore(eventStart, currentTime = new Date()) {
     ...
     */
    static formatTimeDifference(notificationTime, eventStart) {
