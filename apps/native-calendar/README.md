# Solace Calendar (native)

This app is configured for Expo/EAS under the `astralgrove` org, project slug `solace`. It shares auth, E2EE, settings, push and UI primitives with `apps/native-mail` (Solace Mail) through `packages/native-core`.
...
```bash
bun run update:preview      # apps/native-calendar
...
Create `apps/native-calendar/.env` or `apps/native-calendar/.env.local` with:
...
The development client uses `solace-dev://api/auth`. The backend trusts the
...
`build:dev:*` produces a development client with the Calendar Dev icon and `.dev` bundle ID.
...
The workflow publishes through `apps/native-calendar/scripts/publish-update.ts`, which applies the matching `eas.json` build-profile `env` so the fingerprint runtimeVersion matches installed binaries. It also bakes production API origins into published bundles:
