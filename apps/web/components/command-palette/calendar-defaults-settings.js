...
import { Calendar, Check, ArrowLeft, Minimize2, } from "lucide-react";
import { WORKING_DAYS } from "./constants";
export function CalendarDefaultsSettings({ open, onOpenChange, localSettings, updateSetting, goBack, workingDaysList, TransitionContainer, transitionDirection, }) {
      ...
      <TransitionContainer direction={transitionDirection}>
        ...
        <CommandList>
          <CommandGroup heading="Week Settings">
            {WORKING_DAYS.map((day) => (<CommandItem key={day.value} onSelect={() => updateSetting("weekStartDay", day.value)} className="px-4 py-3 hover:bg-accent/20 data-[selected=true]:bg-accent/30">
          ...
          <CommandGroup heading="Working Days">
            {WORKING_DAYS.map((day) => (<CommandItem key={day.value} onSelect={() => {
          ...
          <CommandGroup heading="View Settings">
            <CommandItem onSelect={() => updateSetting("compactView", !localSettings.compactView)} className="px-4 py-3 hover:bg-accent/20 data-[selected=true]:bg-accent/30">
              ...
              {localSettings.compactView && (<Check className="ml-2 h-4 w-4 text-primary"/>)}
            </CommandItem>
          </CommandGroup>
        </CommandList>
      </TransitionContainer>
