...
interface AccountSecuritySectionProps {
  ...
  hasOAuthAccount: boolean;
  hasPasswordAccount: boolean;
  hasOAuthOnlyAccess: boolean;
  changingPassword: boolean;
  settingPassword: boolean;
  resettingEncryptionPassword: boolean;
  handleChangePassword: AccountSettingsProps["handleChangePassword"];
  handleSetPassword: AccountSettingsProps["handleSetPassword"];
  handleResetEncryptionPassword: AccountSettingsProps["handleResetEncryptionPassword"];
}
...
function AccountSecuritySection({
  ...
  const handlePasswordSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!newPassword.trim()) {
      setPasswordMessage({ kind: "error", text: isChangePasswordForm ? "Enter your current password and a new password." : "Enter a new password and confirm it." });
      return;
    }
    ...
    try {
      if (isChangePasswordForm) {
        ...
        setPasswordMessage({ kind: "success", text: "Your email sign-in password has been updated. After email sign-in, Solace will also use it to protect your encryption keys." });
        ...
        setPasswordMessage({ kind: "success", text: "An email sign-in password has been added to your account. OAuth and passkey sign-in still use your separate encryption password unless you reset it below." });
      } else {
        ...
        setPasswordMessage({ kind: "success", text: "Your encryption password has been reset for OAuth and passkey sign-in. This keeps your encrypted data intact and only replaces the password used to unlock your encryption keys on new devices." });
      }
      resetSecurityForm();
      ...
      <div className="px-2 pb-1">
        {hasOAuthAccount ? (
          <div className="mx-1 mb-2 rounded-lg border border-border/50 bg-muted/20 p-3 text-xs leading-relaxed text-muted-foreground">
            ...
            {hasOAuthOnlyAccess ? " Setting an email password adds email sign-in to this account." : " Your email sign-in password stays separate from that encryption password."}{" "}
        ...
        {hasPasswordAccount ? (
          <button type="button" onClick={() => { setActiveSecurityForm("change-password"); setPasswordMessage(null); }} disabled={isBusy || isAnySecurityFormOpen}
            ...
            <div className="flex size-6 shrink-0 items-center justify-center"><Lock className="size-4 text-muted-foreground" /></div>
        ...
        {hasOAuthOnlyAccess ? (
          <button type="button" onClick={() => { setActiveSecurityForm("set-password"); setPasswordMessage(null); }} disabled={isBusy || isAnySecurityFormOpen}
            className="flex w-full items-center gap-3 rounded-md p-2 text-left transition-colors hover:bg-accent/50 focus:bg-accent/50 focus:outline-none disabled:cursor-not-allowed disabled:opacity-60"
            style={{ display: isAnySecurityFormOpen ? "none" : undefined }}>
            <div className="flex size-6 shrink-0 items-center justify-center"><Lock className="size-4 text-muted-foreground" /></div>
            <div className="min-w-0 flex-1">
        ...
        {hasOAuthAccount ? (
          <button type="button" onClick={() => { setActiveSecurityForm("reset-encryption"); setPasswordMessage(null); }} disabled={isBusy || isAnySecurityFormOpen}
            className="flex w-full items-center gap-3 rounded-md p-2 text-left transition-colors hover:bg-accent/50 focus:bg-accent/50 focus:outline-none disabled:cursor-not-allowed disabled:opacity-60"
            style={{ display: isAnySecurityFormOpen ? "none" : undefined }}>
            <div className="flex size-6 shrink-0 items-center justify-center"><RotateCcw className="size-4 text-muted-foreground" /></div>
            <div className="min-w-0 flex-1">
        ...
        <AnimatedCollapse isOpen={isAnySecurityFormOpen}>
          <form className="mx-1 my-1 rounded-lg border border-border/50 bg-muted/20 p-4 space-y-3" onSubmit={handlePasswordSubmit}>
            <InlineMessage msg={passwordMessage} />
            <p className="text-xs leading-relaxed text-muted-foreground">
              {isChangePasswordForm ? "Update your email sign-in password. After email sign-in, Solace also uses it to protect your encryption keys."
                : isSetPasswordForm ? "Add an email sign-in password to this account. This gives you an email/password sign-in option without changing the separate encryption password used by OAuth or passkey sign-in."
            ...
            {isChangePasswordForm ? (
              <FieldInput label="Current password" type="password" value={currentPassword} onChange={setCurrentPassword} autoComplete="current-password" disabled={securityFormBusy} />
            ) : null}
            <FieldInput label={isResetEncryptionForm ? "New encryption password" : "New password"} type="password" value={newPassword} onChange={setNewPassword} autoComplete="new-password" disabled={securityFormBusy} />
            <FieldInput label={isResetEncryptionForm ? "Confirm new encryption password" : "Confirm new password"} type="password" value={confirmPassword} onChange={setConfirmPassword} autoComplete="new-password" disabled={securityFormBusy} />
            <div className="flex gap-2 pt-1">
              <button type="submit" disabled={securityFormBusy} className="inline-flex h-8 items-center gap-2 rounded-md bg-primary px-4 text-xs font-medium text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-60">
                {securityFormBusy ? (<><Loader2 className="size-3 animate-spin" />Saving…</>) : isChangePasswordForm ? "Update Password" : isSetPasswordForm ? "Set Password" : "Reset Encryption Password"}
              </button>
              <button type="button" onClick={() => { setActiveSecurityForm(null); resetSecurityForm(); setPasswordMessage(null); }} disabled={securityFormBusy}
...
function AccountDangerSection({ isBusy, deletingAccount, handleReset, handleDeleteAccount }: AccountDangerSectionProps) {
  const [showResetConfirm, setShowResetConfirm] = useState(false);
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);

  return (
    <>
      <div className="mt-1 border-t border-border/50 px-4 pb-1 pt-2 text-xs font-medium text-muted-foreground">Danger Zone</div>
      <div className="px-2 pb-4">
        <div style={{ display: showResetConfirm || showDeleteConfirm ? "none" : undefined }}>
          <button type="button" onClick={() => setShowResetConfirm(true)} disabled={isBusy}
            ...
            <div className="flex size-6 shrink-0 items-center justify-center"><RotateCcw className="size-4" /></div>
            <div className="min-w-0 flex-1">
          ...
          <button type="button" onClick={() => setShowDeleteConfirm(true)} disabled={isBusy}
            className="flex w-full items-center gap-3 rounded-md p-2 text-left text-destructive transition-colors hover:bg-destructive/8 focus:bg-destructive/10 focus:outline-none disabled:cursor-not-allowed disabled:opacity-60">
            <div className="flex size-6 shrink-0 items-center justify-center"><Trash2 className="size-4" /></div>
            <div className="min-w-0 flex-1">
        ...
        <AnimatedCollapse isOpen={showDeleteConfirm}>
          <div className="mx-1 my-1 rounded-lg border border-destructive/20 bg-destructive/5 p-3 space-y-3">
            <div className="flex items-start gap-2 text-xs text-muted-foreground">
              ...
              <span>This permanently deletes your account, calendars, events, categories, subscriptions, passkeys, and settings. This cannot be undone.</span>
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={() => { handleDeleteAccount(); setShowDeleteConfirm(false); }} disabled={isBusy}
                ...
                {deletingAccount ? <Loader2 className="size-3 animate-spin" /> : <Trash2 className="size-3" />}
                Delete my account
              </button>
              <button type="button" onClick={() => setShowDeleteConfirm(false)} className="inline-flex h-8 items-center rounded-md border border-border bg-background px-3 text-xs font-medium text-foreground transition-colors hover:bg-accent/40">Cancel</button>
            </div>
          </div>
        </AnimatedCollapse>
        <AnimatedCollapse isOpen={showResetConfirm}>
          <div className="mx-1 my-1 rounded-lg border border-destructive/20 bg-destructive/5 p-3 space-y-3">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => { handleReset(); setShowResetConfirm(false); }} disabled={isBusy}
                className="inline-flex h-8 items-center gap-2 rounded-md bg-destructive px-3 text-xs font-medium text-destructive-foreground transition-opacity hover:opacity-90 disabled:opacity-60">
                <Check className="size-3" />
                Yes, reset everything
              </button>
              <button type="button" onClick={() => setShowResetConfirm(false)} className="inline-flex h-8 items-center rounded-md border border-border bg-background px-3 text-xs font-medium text-foreground transition-colors hover:bg-accent/40">Cancel</button>
            </div>
          </div>
        </AnimatedCollapse>
      </div>
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        <AccountSecuritySection
          ...
          hasOAuthOnlyAccess={hasOAuthOnlyAccess}
