...
export const scrollToSelectedTime = (dropdownRef, selectedTime) => {
    if (!dropdownRef.current)
        return;
    // Try to find exact match first
    const selectedButton = dropdownRef.current.querySelector(`[data-time-value="${selectedTime}"]`);
    if (selectedButton) {
    ...
    const closestButton = dropdownRef.current.querySelector(`[data-time-value="${roundedTime}"]`);
    if (closestButton) {
...
export const generateAllTimeOptions = (timeFormat) => {
    const options = [];
