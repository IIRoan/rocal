...
export function RecurringEventOptions({ event, isRecurringInstance, onEditSeries, onEditThisOnly, onEditThisAndFuture, onDeleteSeries, onDeleteThisOnly, onDeleteThisAndFuture, onCancel, mode, onFallbackDelete, }) {
    ...
    const actuallyRecurring = !!(event.recurrence ||
    ...
    if (!actuallyRecurring && onFallbackDelete && mode === 'delete') {
        ...
        <div className="space-y-3">
          <Button variant="destructive" className="w-full" onClick={onFallbackDelete}>
    ...
    return (<div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <Button variant="outline" className="w-full justify-start text-left h-auto py-4 px-4" onClick={() => {
            if (isEdit) {
