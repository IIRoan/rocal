...
interface MailboxListViewProps {
  systemMailboxes: JmapMailbox[];
  customMailboxes: JmapMailbox[];
  onBack: () => void;
  onNavigateTo: (view: string) => void;
  onEditMailbox: (mailbox: JmapMailbox) => void;
}

function MailboxListView({
  systemMailboxes,
  customMailboxes,
  onBack,
  onNavigateTo,
  onEditMailbox,
}: MailboxListViewProps) {
  return (
...
interface MailboxCreateViewProps {
  onBack: () => void;
  onCreateMailbox: (name: string) => Promise<void>;
}

function MailboxCreateView({ onBack, onCreateMailbox }: MailboxCreateViewProps) {
...
interface MailboxEditViewProps {
  editingMailbox: JmapMailbox;
  onBack: () => void;
  onRenameMailbox?: (id: string, name: string) => Promise<void>;
  onDeleteMailbox: (id: string) => Promise<void>;
}

function MailboxEditView({
  editingMailbox,
  onBack,
  onRenameMailbox,
  onDeleteMailbox,
}: MailboxEditViewProps) {
  const [newName, setNewName] = useState(editingMailbox.name);
  const [saving, setSaving] = useState(false);
