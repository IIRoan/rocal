import { containsArmoredPgpMessage } from "@/lib/mail/pgp-layers";
import {
...
function stripHtmlToText(html: string): string {
  const { html: cleanedHtml } = splitHtmlQuote(html);
...
function finalizePreview(raw: string): string {
  ...
  const { body } = splitPlaintextQuote(raw);
...
export function messageNeedsDecryptedPreview(
  message: Pick<
    JmapEmailMessage,
    "attachments" | "bodyStructure" | "bodyValues" | "htmlBody" | "textBody" | "preview"
  >,
): boolean {
