...
export default function LoginPage() {
    ...
    if (isPending || isCheckingSession) {
        return (<section className="min-h-screen flex items-center justify-center px-4">
    ...
    return (<section className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="bg-card border border-border rounded-xl shadow-sm p-6">
          ...
          {error && (<div className="mb-4 p-3 rounded-md bg-destructive/10 border border-destructive/20" role="alert">
          ...
          <div className="space-y-3">
            {isPasskeySupported ? (<>
                {/* GitHub first */}
                <button onClick={handleGitHubLogin} disabled={isLoading} className="w-full inline-flex items-center justify-center gap-2 rounded-md px-4 py-2.5 text-sm font-medium bg-neutral-900 text-white hover:bg-neutral-800 dark:bg-neutral-950 dark:hover:bg-neutral-900 border border-neutral-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" aria-busy={isLoading}>
                  {isLoading ? (<>
                ...
                {/* Passkey second */}
                <button onClick={handlePasskeyLogin} disabled={passkeyLoading} className="w-full inline-flex items-center justify-center gap-2 rounded-md px-4 py-2.5 text-sm font-medium bg-primary text-primary-foreground hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed" aria-busy={passkeyLoading}>
                  {passkeyLoading ? (<>
