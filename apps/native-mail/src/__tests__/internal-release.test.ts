...
describe("internal release manifest", () => {
  it("escapes & in plist IPA URLs so iOS can parse the manifest", () => {
    ...
    const plist = buildManifestPlist({
      ipaUrl,
      bundleId: "onl.solace.mail.dev",
      bundleVersion: "1",
      title: "Solace Mail Dev",
    });
...
describe("release keys", () => {
  it("keeps every artifact under the Solace prefix so shared buckets are never pruned", () => {
    expect(releaseKeys("development")).toEqual({
      ipa: "solace/releases/mail/development.ipa",
      apk: "solace/releases/mail/development.apk",
      plist: "solace/releases/mail/development.plist",
      page: "solace/releases/mail/development.html",
      qr: "solace/releases/mail/development-qr.png",
    });
    expect(releaseKeys("preview").apk).toBe("solace/releases/mail/preview.apk");
  });
