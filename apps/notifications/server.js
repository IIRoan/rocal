...
class NotificationServer {
    ...
    async sendNotification(notification) {
        ...
        try {
            ...
            await this.logNotification(event.id, user.id, notificationType, minutesBefore, "sent");
            console.log(`✅ Sent ${notificationType} notification for event "${event.title}" to ${user.email}`);
        }
        catch (error) {
            console.error(`❌ Failed to send ${notificationType} notification for event "${event.title}" to ${user.email}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    async generateEmailContent(event, user, minutesBefore) {
        const userSettings = user.settings;
     ...
     */
    async renderEmailTemplate(event, user, formattedDetails) {
    ...
    async handleNotificationFailure(notification, result) {
        ...
        await this.logNotification(notification.eventId, notification.event.userId, notification.notificationType, notification.minutesBefore, "failed", result.error);
    }
    /**
     * Process retry queue
     */
    async processRetryQueue() {
        const now = new Date();
        const retryItems = Array.from(this.retryQueue.values()).filter((item) => item.nextRetryAt <= now);
    ...
    async cleanupOldNotifications(retentionDays = 30) {
        ...
        try {
            ...
            console.log(`🧹 Cleanup completed: Deleted ${deletedLogs.count} logs, ${deletedSentNotifications.count} old sent notifications, and ${deletedStaleNotifications.count} stale notifications for past events`);
        }
        catch (error) {
