...
const MailSelectionAnimContext = createContext<SharedValue<number> | null>(null);
...
type ChromeLayer = "outgoing" | "incoming";
 ...
 */
export function headerChromeMotion(p: number, layer: ChromeLayer) {
  "worklet";
  if (layer === "outgoing") {
    return {
      opacity: interpolate(p, [0, 0.35, 0.7, 1], [1, 0.7, 0.15, 0], Extrapolation.CLAMP),
      translateY: interpolate(p, [0, 1], [0, -14], Extrapolation.CLAMP),
      scale: interpolate(p, [0, 1], [1, 0.955], Extrapolation.CLAMP),
    };
  }
  return {
    opacity: interpolate(p, [0, 0.3, 0.65, 1], [0, 0, 0.75, 1], Extrapolation.CLAMP),
 ...
 */
export function bottomChromeMotion(p: number, layer: ChromeLayer) {
  "worklet";
  if (layer === "outgoing") {
    return {
      opacity: interpolate(p, [0, 0.4, 0.75, 1], [1, 0.65, 0.12, 0], Extrapolation.CLAMP),
      translateY: interpolate(p, [0, 1], [0, 28], Extrapolation.CLAMP),
      scale: interpolate(p, [0, 1], [1, 0.82], Extrapolation.CLAMP),
    };
  }
  return {
    opacity: interpolate(p, [0, 0.25, 0.6, 1], [0, 0, 0.7, 1], Extrapolation.CLAMP),
