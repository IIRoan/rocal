...
describe("release keys", () => {
  it("keeps every artifact under the Solace prefix so shared buckets are never pruned", () => {
    expect(releaseKeys("development")).toEqual({
      ipa: "solace/releases/development.ipa",
      apk: "solace/releases/development.apk",
      plist: "solace/releases/development.plist",
      page: "solace/releases/development.html",
      qr: "solace/releases/development-qr.png",
    });
    expect(releaseKeys("preview").apk).toBe("solace/releases/preview.apk");
  });
