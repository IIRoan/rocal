...
export function MobileWeekNav({ currentDate, currentView, onDateChange, onTodayClick, className, }) {
    ...
    return (<div className={cn("sticky top-0 z-40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b border-border md:hidden", className)}>
      <div className="flex items-center justify-between px-4 py-3 safe-area-inset-top">
        <Button variant="ghost" size="sm" onClick={handlePrevious} className="p-2">
          <ChevronLeft size={20}/>
        </Button>
        ...
        <div className="flex-1 text-center">
          ...
          {displayText.sub && (<div className="text-xs text-muted-foreground mt-0.5">
              {displayText.sub}
            </div>)}
        </div>
        
        <div className="flex items-center gap-2">
          {!isCurrentPeriod && (<Button variant="outline" size="sm" onClick={handleToday} className="text-xs px-3 py-1.5 h-auto">
              Today
            </Button>)}
