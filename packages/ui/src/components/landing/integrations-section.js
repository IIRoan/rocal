...
const categories = ["All", "Calendar", "Communication", "Video", "Productivity", "CRM", "Project Management", "Development"];
export function IntegrationsSection() {
    const [selectedCategory, setSelectedCategory] = React.useState("All");
    const filteredIntegrations = selectedCategory === "All"
        ? integrations
        : integrations.filter(integration => integration.category === selectedCategory);
    return (<section className="py-24 px-6 bg-muted/20">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredIntegrations.map((integration) => (<Card key={integration.name} className={`group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-card/50 backdrop-blur-sm border-border/50 relative ${integration.popular ? "ring-1 ring-primary/20" : ""}`}>
              {integration.popular && (<Badge className="absolute -top-2 -right-2 bg-primary text-primary-foreground text-xs">
                  Popular
                </Badge>)}
              
              <CardContent className="p-6">
                <div className="flex items-start gap-4">
                  <div className={`p-3 rounded-lg ${integration.color} text-2xl group-hover:scale-110 transition-transform duration-300`}>
                    {integration.logo}
                  </div>
        ...
        <div className="mt-16 text-center space-y-6">
          <div className="p-8 rounded-2xl bg-gradient-to-r from-primary/10 to-accent/10 border border-border/50">
            ...
            <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">
              We're constantly adding new integrations. Request your favorite tools
              or use our powerful API to build custom connections.
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-8">
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-muted-foreground">Sync Reliability</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-success mb-2">&lt;1min</div>
