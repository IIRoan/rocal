...
import { Command, CommandEmpty, CommandGroup, CommandInput, CommandItem, CommandList, } from "@workspace/ui/components/navigation/command";
...
export function ShadcnAutocomleteTimePicker({ value, onChange, is24Hour = false, locale = "en-US", timeZone = currentTimezone, placeholder = "Select time...", }) {
    ...
    const selectedIndex = timeOptions.findIndex(time => time.getHours() === selectedTime.getHours() &&
    ...
    const handleCustomTimeInput = (inputValue) => {
    ...
    const isValidCustomTime = (inputValue) => {
        const timeRegex = /^(\d{1,2}):(\d{1,2})$/;
        const match = inputValue.match(timeRegex);
        if (match && match[1] && match[2]) {
            const hours = parseInt(match[1], 10);
    ...
    const scrollToSelectedItem = () => {
        if (selectedIndex !== -1 && commandListRef.current && timeOptions[selectedIndex]) {
            const selectedElement = commandListRef.current.querySelector(`[data-value="${formatTime(timeOptions[selectedIndex])}"]`);
            if (selectedElement) {
    ...
    return (<Popover open={open} onOpenChange={(newOpen) => {
            setOpen(newOpen);
            if (!newOpen) {
                setSearchValue("");
            }
      ...
      <PopoverContent className="w-[200px] p-0" onOpenAutoFocus={(e) => {
            e.preventDefault();
            // Scroll to selected item after popover opens
            setTimeout(scrollToSelectedItem, 100);
        }}>
        <Command>
          <CommandInput placeholder="Search or type time (e.g. 0:05)..." data-testid="CommandInput" value={searchValue} onValueChange={setSearchValue} onKeyDown={(e) => {
            if (e.key === 'Enter') {
          ...
          <CommandList ref={commandListRef} className="max-h-[200px] overflow-y-auto" onWheel={(e) => e.stopPropagation()}>
            <CommandEmpty>No time found.</CommandEmpty>
            {searchValue && isValidCustomTime(searchValue) && (<CommandGroup heading="Custom Time">
                <CommandItem value={`custom-${searchValue}`} onSelect={() => {
            ...
            const isSelected = time.getHours() === selectedTime.getHours() &&
                time.getMinutes() === selectedTime.getMinutes();
            return (<CommandItem key={index} value={timeString} onSelect={() => handleSelect(time)}>
