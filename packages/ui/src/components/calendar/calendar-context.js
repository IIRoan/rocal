...
export function CalendarProvider({ children, initialCalendars = [], onCreateCalendar, onUpdateCalendar, onRefreshCalendars, }) {
    ...
    const loadVisibilityFromStorage = () => {
        if (typeof window === "undefined")
            return {};
        try {
    ...
    const saveVisibilityToStorage = (state) => {
        if (typeof window === "undefined")
            return;
        try {
    ...
    useEffect(() => {
        const visibleCalendars = calendars.filter((cal) => localVisibilityState[cal.id] ?? cal.isVisible);
    ...
    const toggleCalendarVisibility = useCallback(async (calendarId) => {
    ...
    const toggleColorVisibility = (color) => {
    ...
    const isColorVisible = (color) => {
        if (!color)
            return true;
        return visibleColors.includes(color);
    };
    // Custom setCurrentDate that also persists to localStorage
    const setCurrentDateWithPersistence = useCallback((date) => {
