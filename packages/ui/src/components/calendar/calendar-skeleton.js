...
export function CalendarSkeleton({ view = "month", className, compactView = false, }) {
    const today = new Date();
    const weekStart = startOfWeek(today, { weekStartsOn: 0 });
    const daysOfWeek = Array.from({ length: 7 }, (_, i) => format(addDays(weekStart, i), 'EEE'));
    if (view === "month") {
        ...
        <div className="space-y-0 border border-border rounded-lg overflow-hidden">
          ...
          <div className="grid grid-cols-7 border-b border-border bg-muted/30">
            {daysOfWeek.map((day, i) => (<div key={i} className="p-2 text-center border-r border-border last:border-r-0">
                <Skeleton className="h-4 w-8 mx-auto" variant="shimmer"/>
              </div>))}
          </div>

          {/* Calendar cells with realistic event distribution */}
          {Array.from({ length: 6 }).map((_, weekIndex) => (<div key={weekIndex} className="grid grid-cols-7 border-b border-border last:border-b-0">
              {Array.from({ length: 7 }).map((_, dayIndex) => {
                    const cellIndex = weekIndex * 7 + dayIndex;
                    const hasEvents = cellIndex % 3 === 0 || cellIndex % 5 === 0;
                    const eventCount = hasEvents ? Math.floor(Math.random() * 3) + 1 : 0;
                    return (<div key={dayIndex} className="h-24 p-2 border-r border-border last:border-r-0 space-y-1" style={{ animationDelay: `${(weekIndex * 7 + dayIndex) * 0.02}s` }}>
                    <Skeleton className="h-4 w-6" variant="shimmer"/>
                    {/* Realistic event skeletons */}
                    {Array.from({ length: eventCount }).map((_, eventIndex) => (<Skeleton key={eventIndex} className={`h-4 ${eventIndex === 0 ? 'w-full' : eventIndex === 1 ? 'w-3/4' : 'w-1/2'}`} variant="wave" style={{ animationDelay: `${(weekIndex * 7 + dayIndex) * 0.02 + eventIndex * 0.1}s` }}/>))}
                  </div>);
                })}
            </div>))}
        </div>
    ...
    if (view === "week") {
        ...
        <div className="flex flex-col border border-border rounded-lg overflow-hidden">
          ...
          <div className="grid grid-cols-8 border-b border-border bg-muted/30">
            ...
            {daysOfWeek.map((day, i) => (<div key={i} className="p-3 text-center border-r border-border last:border-r-0">
          ...
          <div className="flex-1 overflow-hidden">
            {Array.from({ length: 24 }).map((_, hourIndex) => (<div key={hourIndex} className={`grid grid-cols-8 border-b border-border last:border-b-0 ${compactView ? 'min-h-[34px]' : 'min-h-[40px]'}`}>
                <div className="p-2 border-r border-border bg-muted/10">
                  <Skeleton className="h-4 w-12" variant="shimmer"/>
                </div>
                {Array.from({ length: 7 }).map((_, dayIndex) => {
                    const hasEvent = (hourIndex >= 9 && hourIndex <= 17) && Math.random() > 0.85;
                    return (<div key={dayIndex} className="p-1 border-r border-border last:border-r-0 relative">
                      {hasEvent && (<Skeleton className={`${compactView ? 'h-9' : 'h-12'} w-full rounded`} variant="wave" style={{ animationDelay: `${hourIndex * 0.05 + dayIndex * 0.02}s` }}/>)}
                    </div>);
                })}
              </div>))}
          </div>
    ...
    if (view === "day") {
        ...
        <div className="flex border border-border rounded-lg overflow-hidden">
          ...
          <div className="w-20 border-r border-border bg-muted/10">
            {Array.from({ length: 24 }).map((_, hourIndex) => (<div key={hourIndex} className={`${compactView ? 'h-9' : 'h-11'} p-2 border-b border-border last:border-b-0`}>
                <Skeleton className="h-4 w-12" variant="shimmer"/>
              </div>))}
          </div>
          ...
          <div className="flex-1">
            ...
            <div className="relative">
              {Array.from({ length: 24 }).map((_, hourIndex) => {
                ...
                return (<div key={hourIndex} className={`${compactView ? 'h-9' : 'h-11'} p-2 border-b border-border last:border-b-0 relative`}>
                    {hasEvent && (<Skeleton className={`w-4/5 rounded absolute left-2 top-2`} style={{ height: `${eventLength * 60}px` }} variant="wave"/>)}
                  </div>);
            })}
    ...
    if (view === "agenda") {
        ...
        <div className="space-y-6 px-4">
          {Array.from({ length: 8 }).map((_, dayIndex) => {
                ...
                return (<div key={dayIndex} className="space-y-3 animate-slide-in" style={{ animationDelay: `${dayIndex * 0.1}s` }}>
                {/* Date header */}
                ...
                <div className="space-y-3 pl-6">
                  {Array.from({ length: eventCount }).map((_, eventIndex) => (<div key={eventIndex} className="flex items-start gap-4 p-3 rounded-lg border border-border animate-scale-in" style={{ animationDelay: `${dayIndex * 0.1 + eventIndex * 0.05}s` }}>
                      ...
                      <div className="flex-1 space-y-1">
                        ...
                        {eventIndex % 3 === 0 && (<Skeleton className="h-3 w-1/2" variant="wave"/>)}
                      </div>
...
export function SidebarCalendarSkeleton() {
    return (<div className="space-y-4 p-4 animate-fade-in">
      ...
      <div className="space-y-1 animate-slide-in" style={{ animationDelay: '0.1s' }}>
        ...
        <div className="grid grid-cols-7 gap-1 mb-2">
          {Array.from({ length: 7 }).map((_, i) => (<Skeleton key={i} className="h-3 w-full text-center" variant="shimmer"/>))}
        </div>
        
        {/* Calendar dates */}
        {Array.from({ length: 6 }).map((_, weekIndex) => (<div key={weekIndex} className="grid grid-cols-7 gap-1">
            {Array.from({ length: 7 }).map((_, dayIndex) => {
                const isToday = weekIndex === 2 && dayIndex === 3; // Mock today
                return (<Skeleton key={dayIndex} className={`h-6 w-6 ${isToday ? 'rounded-full' : 'rounded'}`} variant={isToday ? "shimmer" : "wave"} style={{ animationDelay: `${(weekIndex * 7 + dayIndex) * 0.02}s` }}/>);
            })}
          </div>))}
      </div>
      ...
      <div className="space-y-3 animate-slide-in" style={{ animationDelay: '0.2s' }}>
        <Skeleton className="h-4 w-24" variant="shimmer"/>
        {Array.from({ length: 3 }).map((_, i) => (<div key={i} className="flex items-center gap-3 p-2 rounded hover:bg-accent/50 transition-colors">
      ...
      <div className="space-y-3 animate-slide-in" style={{ animationDelay: '0.3s' }}>
        <Skeleton className="h-4 w-20" variant="shimmer"/>
        {Array.from({ length: 4 }).map((_, i) => (<div key={i} className="flex items-center gap-3 p-2 rounded hover:bg-accent/50 transition-colors">
