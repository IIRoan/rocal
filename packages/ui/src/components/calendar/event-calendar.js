...
import { addDays, addMonths, addWeeks, endOfWeek, format, isSameMonth, startOfWeek, subMonths, subWeeks, startOfMonth, endOfMonth, eachDayOfInterval, } from "date-fns";
...
export function EventCalendar({ className, initialView = "month", events = [], categories = [], loading = false, eventsLoading = false, error = null, onCreateEvent, onUpdateEvent, onDeleteEvent, onCreateCategory, onDateRangeChange, showWeekNumbers = false, compactView = false, timeFormat = "24h", defaultReminder = null, defaultEventDuration = 60, defaultCalendarId = null, weekStartDay = 1, workingDays = [1, 2, 3, 4, 5], timezone, themeSettings, onLoadNotifications, onUpdateNotifications, onEventEdit, hideSidebarTrigger = false, onSidebarToggle, }) {
    ...
    const handleEventCreate = (startTime) => {
        ...
        const newEvent = {
            id: undefined, // This ensures it's treated as a new event
            title: "",
            start: startTime,
    ...
    const handleEventSave = async (event) => {
        try {
            ...
            const isNetworkError = error?.error === "Network Error" ||
                error?.statusCode === 0 ||
                !navigator.onLine;
            if (isNetworkError) {
    ...
    const handleEventUpdate = async (updatedEvent) => {
        try {
            ...
            toast.success(`Event "${updatedEvent.title}" moved`, {
                description: format(new Date(updatedEvent.start), "MMM d, yyyy 'at' h:mm a"),
                position: "bottom-left",
            });
        }
        catch (error) {
    ...
    return (<ErrorBoundary onError={(error, errorInfo) => {
      ...
      <div className="flex has-data-[slot=month-view]:flex-1 flex-col rounded-lg" style={{
        ...
        <CalendarDndProvider onEventUpdate={handleEventUpdate} timezone={timezone}>
          <div className={cn("sticky top-0 z-50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-2 py-5 sm:px-4", className)}>
            ...
            try {
                ...
                return (<SidebarTrigger className="peer size-7 text-muted-foreground/80 hover:text-foreground/80 hover:bg-transparent! sm:-ms-1.5 lg:data-[state=invisible]:opacity-0 lg:data-[state=invisible]:pointer-events-none transition-opacity ease-in-out duration-200" isOutsideSidebar/>);
            }
            catch {
                return null;
            }
        })()}
                {onSidebarToggle && (<Button variant="ghost" size="icon" className="peer size-7 text-muted-foreground/80 hover:text-foreground/80 hover:bg-transparent! sm:-ms-1.5" onClick={onSidebarToggle}>
                ...
                <h2 className={cn("font-semibold text-xl transition-transform ease-in-out duration-300", !hideSidebarTrigger && "lg:peer-data-[state=invisible]:-translate-x-7.5")}>
                  {viewTitle}
                </h2>
              </div>
            </div>
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center sm:gap-2 max-sm:order-1">
                  <Button variant="ghost" size="icon" className="max-sm:size-8" onClick={handlePrevious} aria-label="Previous" disabled={loading}>
                    <ChevronLeftIcon size={16} aria-hidden="true"/>
                  </Button>
                  <Button variant="ghost" size="icon" className="max-sm:size-8" onClick={handleNext} aria-label="Next" disabled={loading}>
                    <ChevronRightIcon size={16} aria-hidden="true"/>
                  </Button>
                </div>
                <Button className="max-sm:h-8 max-sm:px-2.5! bg-accent hover:bg-accent/80 text-accent-foreground" onClick={handleToday} disabled={loading}>
                  Today
                </Button>
              </div>
              <div className="flex items-center justify-between gap-2">
                <Button variant="outline" className="max-sm:h-8 max-sm:px-2.5!" onClick={() => {
            ...
            handleEventCreate(startTime);
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" className="gap-1.5 max-sm:h-8 max-sm:px-2! max-sm:gap-1" disabled={loading}>
          ...
          <div className="flex flex-1 flex-col relative">
            ...
            {eventsLoading && events && events.length > 0 && (<EventLoadingSkeleton view={view} compactView={compactView} className="absolute inset-0 z-10"/>)}
            
            {view === "month" && (<MonthView currentDate={currentDate} events={events} onEventSelect={handleEventSelect} onEventCreate={handleEventCreate} showWeekNumbers={showWeekNumbers} compactView={compactView} timeFormat={timeFormat} weekStartDay={weekStartDay} workingDays={workingDays} timezone={timezone}/>)}
            {view === "week" && (<WeekView currentDate={currentDate} events={events} onEventSelect={handleEventSelect} onEventCreate={handleEventCreate} compactView={compactView} timeFormat={timeFormat} weekStartDay={weekStartDay} workingDays={workingDays} timezone={timezone}/>)}
            {view === "day" && (<DayView currentDate={currentDate} events={events} onEventSelect={handleEventSelect} onEventCreate={handleEventCreate} compactView={compactView} timeFormat={timeFormat} timezone={timezone}/>)}
            {view === "agenda" && (<AgendaView currentDate={currentDate} events={events} onEventSelect={handleEventSelect} timeFormat={timeFormat} timezone={timezone}/>)}
          </div>
