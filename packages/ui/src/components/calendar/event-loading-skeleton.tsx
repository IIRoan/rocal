        ...
        <div className="grid grid-cols-7 gap-1 animate-pulse">
          {Array.from({ length: 42 }).map((_, i) => {
            ...
            return (
              <div
                key={i}
                className="h-24 p-1 border border-border/30 space-y-1"
              >
                ...
                {Array.from({ length: eventCount }).map((_, eventIndex) => (
                  <Skeleton
                    ...
                    variant="wave"
                    style={{
                      animationDelay: `${i * 0.01 + eventIndex * 0.1}s`,
                    }}
                  />
                ))}
              </div>
      ...
      <div className={cn("relative", className)}>
        ...
        <div className="flex flex-col animate-pulse">
          ...
          {Array.from({ length: 24 }).map((_, hourIndex) => (
            <div
              key={hourIndex}
              className={`grid grid-cols-8 border-b border-border/30 ${compactView ? "min-h-[51px]" : "min-h-[60px]"}`}
            >
              {/* Time column */}
              ...
              {Array.from({ length: 7 }).map((_, dayIndex) => {
                ...
                return (
                  <div
                    key={dayIndex}
                    className="p-1 border-r border-border/30 relative"
                  >
                    {hasEvent && (
                      <Skeleton
                        ...
                        variant="wave"
                        style={{
                          animationDelay: `${hourIndex * 0.02 + dayIndex * 0.01}s`,
                        }}
                      />
                    )}
                  </div>
        ...
        <div className="flex animate-pulse">
          ...
          <div className="w-20 border-r border-border/30">
            {Array.from({ length: 24 }).map((_, hourIndex) => (
              <div
                key={hourIndex}
                className={`${compactView ? "h-9" : "h-11"} p-2 border-b border-border/30`}
              >
          ...
          <div className="flex-1 relative">
            {Array.from({ length: 24 }).map((_, hourIndex) => {
              ...
              return (
                <div
                  key={hourIndex}
                  className={`${compactView ? "h-9" : "h-11"} p-2 border-b border-border/30 relative`}
                >
                  {hasEvent && (
                    <Skeleton
            ...
            <div key={dayIndex} className="space-y-3 animate-pulse">
              ...
              <div className="space-y-2 pl-6">
                {Array.from({ length: eventCount }).map((_, eventIndex) => (
                  <div
                    key={eventIndex}
                    className="flex items-center gap-4 p-3 rounded border border-border/30"
                  >
