...
function oklchToLinearSrgb(oklchStr: string): {
  r: number;
  g: number;
  b: number;
} {
...
describe("WCAG AA: semantic color pairs - light theme", () => {
  ...
  it("foreground on background ≥ 4.5:1", () => {
    expect(
      wcagContrastRatio(c.foreground, c.background),
    ).toBeGreaterThanOrEqual(WCAG_AA);
  });

  it("foreground on card ≥ 4.5:1", () => {
    expect(wcagContrastRatio(c.foreground, c.card)).toBeGreaterThanOrEqual(
      WCAG_AA,
    );
  });
  ...
  it("primaryForeground on primaryBase ≥ 4.5:1", () => {
    expect(
      wcagContrastRatio(c.primaryForeground, c.primaryBase),
    ).toBeGreaterThanOrEqual(WCAG_AA);
  });

  it("secondaryForeground on secondary ≥ 4.5:1", () => {
    expect(
      wcagContrastRatio(c.secondaryForeground, c.secondary),
    ).toBeGreaterThanOrEqual(WCAG_AA);
  });

  it("mutedForeground on background ≥ 4.5:1", () => {
    expect(
      wcagContrastRatio(c.mutedForeground, c.background),
    ).toBeGreaterThanOrEqual(WCAG_AA);
  });

  it("accentForeground on accent ≥ 4.5:1", () => {
    expect(
      wcagContrastRatio(c.accentForeground, c.accent),
    ).toBeGreaterThanOrEqual(WCAG_AA);
  });
});
