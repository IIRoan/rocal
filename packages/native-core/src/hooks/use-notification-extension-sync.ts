...
/** Which secrets an app's notification extension needs; share nothing more than its pushes require. */
export interface NotificationExtensionSecrets {
  /** Session cookie so the extension can fetch sender and subject for mail pushes. */
  mailAuth: boolean;
  /** Notification key so the extension can decrypt reminder titles. */
...
export function useNotificationExtensionSync({
  mailAuth,
  notificationKey,
}: NotificationExtensionSecrets): void {
  ...
  }, [isAuthenticated, isLoading, mailAuth]);
  ...
  useEffect(() => {
    ...
    void runWithAccountKey(async (accountKey) => {
      const exportedKey = await exportNotificationKey(accountKey);
      if (!cancelled) {
        await writeNotificationExtensionKey(exportedKey);
      }
    }).catch(() => undefined);
    return () => {
      cancelled = true;
    };
  }, [isAuthenticated, isEnabled, isReady, notificationKey, runWithAccountKey]);
}
