...
/** Each app maps the pushes it receives to its own routes and cache keys. */
export interface PushTapHandler {
  route: (data: PushTapData) => string | null;
  invalidate: (queryClient: QueryClient, data: PushTapData) => void;
}
