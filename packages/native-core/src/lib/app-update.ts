...
export type AppUpdateRuntimeInfo = {
  ...
  /** Display name from the app config, e.g. "Calendar Dev" or "Solace Mail". */
  appName: string;
...
export function updateDiagnosticsTitle(info: AppUpdateRuntimeInfo): string {
  if (info.appVariant === "development") return info.appName;
...
export function updateDiagnosticsBody(info: AppUpdateRuntimeInfo): string {
  ...
  const lines = [
    info.appVariant === "development"
      ? `This binary is ${info.appName}${info.bundleId ? ` (\`${info.bundleId}\`)` : ""}.`
      : `This is the production ${info.appName} binary.`,
    "",
    `Channel: ${channel}`,
  ...
  if (!info.updatesEnabled) {
    ...
    if (info.executionEnvironment === "storeClient") {
      lines.push(
        `Expo Go cannot install ${info.appName} OTAs. Use a production or development build.`,
      );
    } else if (info.appVariant === "development") {
      lines.push(
        `EAS Updates are off while Metro is serving JavaScript. Quit the bundler and open ${info.appName} from the home screen to receive OTAs on the development channel.`,
      );
    } else {
      lines.push(
